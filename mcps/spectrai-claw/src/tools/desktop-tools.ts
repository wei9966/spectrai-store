...
import {
  ...
  NEAR_MONO_MAX_LUMINANCE_VARIANCE,
  NEAR_MONO_MAX_UNIQUE,
  resolveCaptureBlankDecision,
  resolveHidClickTypeForActivatable,
  resolveScreenshotCaptureMode,
  shouldFallbackClickAfterUia,
  SUSPICIOUS_BLANK_MAX_LUMINANCE_VARIANCE,
  SUSPICIOUS_BLANK_MAX_UNIQUE,
...
function elementActionabilityScore(el: AnnotatedElement): number {
  ...
  let score = 0
  if (cap.supportedActions.some(a => a !== 'focus' && a !== 'clickFallback')) score += 100
  if (cap.backgroundInvoke) score += 20
...
function sortElementsForDisplay(elements: AnnotatedElement[]): AnnotatedElement[] {
  return elements
...
function formatElementLine(e: AnnotatedElement): string {
...
function rankUiaFindResults(stdout: string): string {
  ...
  const items = (Array.isArray(parsed) ? parsed : [parsed]).filter(Boolean) as Record<string, unknown>[]
  const annotated = items.map((it) => {
    const patterns = Array.isArray(it.Patterns)
      ? (it.Patterns as unknown[]).map(String)
      : it.Patterns
        ? [String(it.Patterns)]
        : []
    const cap = inferElementCapability({
...
`
  ...
  async function readActivationTitles(processId?: number): Promise<{ windowTitle: string; foregroundTitle: string }> {
...
`
    ...
    const line = result.stdout
      ...
      .reverse()
      .find(l => l.startsWith(marker))
    if (!line) return { windowTitle: '', foregroundTitle: '' }
    try {
      const parsed = JSON.parse(line.slice(marker.length)) as Partial<{ windowTitle: string; foregroundTitle: string }>
      return {
   ...
   */
  async function probeActivationLocalEvidence(element: AnnotatedElement): Promise<{
    elementGone: boolean
...
`
    try {
      ...
      const line = result.stdout
        ...
        .reverse()
        .find(l => l.startsWith(marker))
      if (!line) return { elementGone: false, targetNameOutsideSelection: false }
      const parsed = JSON.parse(line.slice(marker.length)) as Partial<{
        elementGone: boolean
        targetNameOutsideSelection: boolean
      }>
      return {
...
`
  ...
  registerTool(
    ...
    async (args) => {
      ...
      let flags: string
      switch (button) {
        case 'right': flags = clickType === 'double' ? '0x0008;0x0010;0x0008;0x0010' : '0x0008;0x0010'; break
        case 'middle': flags = clickType === 'double' ? '0x0020;0x0040;0x0020;0x0040' : '0x0020;0x0040'; break
        default: flags = clickType === 'double' ? '0x0002;0x0004;0x0002;0x0004' : '0x0002;0x0004'; break
      }
...
`
  ...
  registerTool(
    'keyboard_hotkey',
    'Press a keyboard shortcut/hotkey combination (e.g., Ctrl+C, Alt+F4, Ctrl+Shift+S).',
    {
      type: 'object',
      properties: {
        keys: {
...
`
  ...
  registerTool(
    'uia_find_element',
    'Advanced: Find UI elements by name/automationId/className via Windows UI Automation. Returns coordinates and state. Note: For Chrome/Electron web content, prefer screenshot + click_element instead.',
    {
      type: 'object',
      properties: {
    ...
    async (args) => {
      ...
      const condVarNames: string[] = []

      if (args.name) {
        conditions.push(`$nameCondition = New-Object Windows.Automation.PropertyCondition([Windows.Automation.AutomationElement]::NameProperty, '${sp(args.name as string)}')`)
        condVarNames.push('$nameCondition')
      }
      if (args.automationId) {
        conditions.push(`$idCondition = New-Object Windows.Automation.PropertyCondition([Windows.Automation.AutomationElement]::AutomationIdProperty, '${sp(args.automationId as string)}')`)
        condVarNames.push('$idCondition')
      }
      if (args.className) {
        conditions.push(`$classCondition = New-Object Windows.Automation.PropertyCondition([Windows.Automation.AutomationElement]::ClassNameProperty, '${sp(args.className as string)}')`)
...
`
  ...
  registerTool(
    'uia_get_tree',
    'Advanced: Get the full UI element tree for a window via UIA. Useful for understanding native app layout. For web apps, screenshot annotation is more reliable.',
    {
      type: 'object',
      properties: {
...
`
  ...
  registerTool(
    'render_hud',
    'Render a sci-fi HUD (heads-up display) overlay onto a screenshot.\n\n' +
    ...
    async (args) => {
      ...
      const highlightMode = (args.highlightMode === 'pulse' || args.highlightMode === 'flash')
        ? (args.highlightMode as 'pulse' | 'flash')
        : 'glow'
