...
export function registerDarwinDesktopTools() {
                    ...
                    if (frontPid) {
                        ...
                        if (axResult.success && getArray(axResult.data, 'elements').length > 0) {
                            for (const el of getArray(axResult.data, 'elements')) {
                                ...
                                if (elCx < originX || elCx >= originX + capW)
                                    continue;
                                if (elCy < originY || elCy >= originY + capH)
                                    continue;
                                // Filter: skip too small or too large
                                if (el.bounds.width <= 2 || el.bounds.height <= 2)
                                    continue;
                                if (el.bounds.width > capW || el.bounds.height > capH)
                                    continue;
                ...
                if (elements.length < 10) {
                    ...
                    if (ocrResult.success && getArray(ocrResult.data, 'results').length > 0) {
                        for (const r of getArray(ocrResult.data, 'results')) {
                            ...
                            if (cx < originX || cx >= originX + capW)
                                continue;
                            if (cy < originY || cy >= originY + capH)
                                continue;
                            elements.push({
                                number: idx,
                                name: r.text || '',
    ...
    // 8. keyboard_hotkey
    // ──────────────────────────────────────────────────────────────────────
    registerTool('keyboard_hotkey', 'Press a keyboard shortcut/hotkey combination (e.g., Cmd+C, Ctrl+C, Alt+F4, Cmd+Shift+S).\n' +
    ...
    // ──────────────────────────────────────────────────────────────────────
    registerTool('uia_find_element', 'Advanced: Find UI elements by name/role/identifier via macOS Accessibility API. Returns coordinates and state. Note: For Chrome/Electron web content, prefer screenshot + click_element instead.', {
        type: 'object',
        properties: {
        ...
        const searchRole = args.className ? args.className.toLowerCase() : null;
    ...
    // ──────────────────────────────────────────────────────────────────────
    registerTool('uia_get_tree', 'Advanced: Get the full UI element tree for a window via macOS Accessibility API. Useful for understanding native app layout. For web apps, screenshot annotation is more reliable.', {
        type: 'object',
        properties: {
                    ...
                    if (frontPid) {
                        ...
                        if (axResult.success && getArray(axResult.data, 'elements').length > 0) {
                            for (const el of getArray(axResult.data, 'elements')) {
                                ...
                                const elCx = Math.round(el.bounds.x + el.bounds.width / 2);
                                const elCy = Math.round(el.bounds.y + el.bounds.height / 2);
                                if (elCx < zx || elCx >= zx + zw)
                                    continue;
                                if (elCy < zy || elCy >= zy + zh)
                                    continue;
                                if (el.bounds.width <= 2 || el.bounds.height <= 2)
                                    continue;
                                if (el.bounds.width > zw || el.bounds.height > zh)
                                    continue;
                                const label = el.title || el.description || el.role || '';
                                const isClickable = /button|link|menuitem|tab|checkbox|radio|combobox|slider|image/i.test(el.role || '');
                                if (!label && !isClickable)
